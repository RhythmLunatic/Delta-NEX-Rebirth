...
local perfects = 	STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_W1")+
		STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_W2")+
		STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_CheckpointHit");

local greats = 	STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_W3");
local goods = 	STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_W4");
local bads = 	STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_W5");

local misses = 	STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetTapNoteScores("TapNoteScore_Miss")+
...
local score = 	scorecap(STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetScore());

local dancepoints;

if string.find(PREFSMAN:GetPreference("VideoRenderers"), "d3d,") then
	dancepoints = math.floor(STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetPercentDancePoints()*100)
else
	dancepoints = (math.floor((STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetPercentDancePoints())*1000000))/10000
end
...
--local grade = ToEnumShortString(STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetGrade());
...
t[#t+1] = LoadFont("venacti/_venacti 26px bold diffuse")..{
	...
	OnCommand=cmd(x,-75;y,110;diffusealpha,0;decelerate,0.3;diffusealpha,1);
	Text="ACCURACY";
}
